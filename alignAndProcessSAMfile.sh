...
rm -r ${tmpfolderToSave}/${name}_templog

# Align the fastq files
echo "Aligning fastq files using STAR align"
STAR --runThreadN 12 --runMode alignReads --outMultimapperOrder Random --outSAMmultNmax 1 --outSAMattributes MD --genomeDir /home/shared/GenomeReference/Long_RefSeqAnnotated_hg38_STARindex --readFilesIn ${datafolderToSave}/${name}_L001_R1_001.fastq ${datafolderToSave}/${name}_L001_R2_001.fastq --outTmpDir ${tmpfolderToSave}/${name}_templog --outFileNamePrefix ${tmpfolderToSave}/${name}_ --alignEndsType Local --scoreGap -1000000 --scoreDelBase -1 --scoreInsBase -1 --outFilterMismatchNmax 999 --outFilterMismatchNoverLmax 999
